...
def load_index_html() -> str:
    ...
    try:
        ...
        log_event(f"Failed to load template {template_path}: {exc}")
        return """<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>AUTOLOAD CMD</title><style>body{margin:0;background:#070b12;color:#d9e2f2;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;padding:28px} .box{max-width:760px;margin:40px auto;border:1px solid #233551;background:#0b1320;border-radius:14px;padding:20px} h1{margin:0 0 8px 0;font-size:22px} p{color:#9bb2d7;line-height:1.6}</style></head><body><div class="box"><h1>UI template not found</h1><p>The server is running but <code>s/templates/index.html</code> is missing in this deployment package.</p><p>Please include <code>s/templates</code> and <code>s/static</code> in your deployment output, or open <code>/?ui=admin</code> if Admin UI is available.</p></div></body></html>"""
